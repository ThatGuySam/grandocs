...
version: "2.3"
lang: en
sourceUrl: "https://help.malighting.com/grandMA3/2.3/HTML/qsg_multicue_sequence.html"
scrapedAt: "2026-06-15T18:25:41.145Z"
pagefind: false
---
...
Because of the differences in our preset, your result will look different than mine. That is okay. It is meant as a demonstration, and we will use the sequence to look at different functions with sequences. The actual preset values are not important.

## Create the Sequence

We begin by getting some values into our programmer. Start by clearing the programmer. Now select Group 7 (all the Mega Pointe) and turn them to 100%. Select a position preset and color preset.

This is our first cue in a new sequence, so let us store it: Press Store and then the executor button 206.

Now, we have a new sequence (number 5), and executor 206 controls the sequence.
...
![](/img/grandma3/2-3/qsg_15_sequence_first-cue_v2-3-b3519d.png)
...
![](/img/grandma3/2-3/qsg_15_sequence_cue-four_v2-3-4b182f.png)
...
Press Time 2 / 5 Please. This changed the current cues time. We could have specified a cue number, and you can do this if you want to change a different cue than the current one.
...
![](/img/grandma3/2-3/qsg_15_sequence_cue-seven_v2-3-fedf6a.png)
...
![Update this description text.](/img/grandma3/2-3/qsg_15_sequence_store-cue-6-popup_v2-3-34655b.png)
...
![](/img/grandma3/2-3/qsg_15_sequence_cue-part-2_v2-3-cb6730.png)
...
![](/img/grandma3/2-3/qsg_15_sequence_cue-named_v2-3-b22ccb.png)
...
![](/img/grandma3/2-3/qsg_15_appearance-pool_v2-3-e22748.png)
...
![](/img/grandma3/2-3/qsg_15_cue-part-appearance_v2-1-a06cef.png)

Close the settings.

The easiest way to assign the appearances to cues (or technically the cue parts) is by using the Swipey menu in the Appearance pool. If you forgot about Swipey, please revisit [chapter 6](/grandma3/2-3/qsg_group/).
...
![](/img/grandma3/2-3/qsg_15_sequence-sheet_colors_v2-3-13712d.png)
...
![](/img/grandma3/2-3/qsg_15_sequence-sheet_tracking_v2-3-f44071.png)

Sequence Sheet in Track Sheet Mode

Here, we can see how an attribute changes through the cues. For instance, the dimmer value of fixture 201 is at 100% (Open) in cue 1 and then at 0% (Closed) in cue 2. This then tracks to cue 6, where the value is at 80%.
...
![](/img/grandma3/2-3/qsg_15_sequence-sheet_tracking_feature-sort_v2-3-45feb8.png)
...
![](/img/grandma3/2-3/qsg_15_sequence-sheet_tracking_final_v2-3-56984f.png)
...
You might have noticed when running the cues that the fixtures faded to a new position and a new color, together with the dimmer values. If we want the fixtures to already be at the next position and with the color and gobos ready, then you could store the color, gobo, and position values in the previous cue, but we can also make the software work for us. It is a function called MIB (Move In Black).
...
![](/img/grandma3/2-3/qsg_15_sequence-sheet_mib_v2-3-aa27ea.png)
...
There are more details and things to know about tracking and MIB, but it is outside the scope of this quick start.
...
There is a whole section of the manual dedicated to cues and sequences. I have linked to it in chapter 8, but here it is again - [Cue and Sequences section](/grandma3/2-3/cue_sequence/). This also has a topic about [Tracking](/grandma3/2-3/cue_tracking/).

We had a short look at MIB. There are many settings related to this, which can be quite complex when Phaser steps are included. If you want to, you can read more about it in the [Move In Black topic](/grandma3/2-3/cue_mib/).

The [next chapter](/grandma3/2-3/qsg_output_dmx/) explores how we can output DMX from the system.
