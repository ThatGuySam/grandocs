...
description: "There are some settings used for storing objects such as cues, presets, and groups."
product: grandma3
version: "2.4"
lang: en
sourceUrl: "https://help.malighting.com/grandMA3/2.4/HTML/cue_store_settings_preferences.html"
scrapedAt: "2026-06-12T21:24:55.533Z"
---
There are some settings used for storing objects such as cues, presets, and groups.
...
In this topic, the settings regarding storing cues are described. Some of these settings will impact how other cues will respond. Elements related to tracking are described in the ["What is Tracking" topic]() and its subtopics.
...
![The image shows the Store Settings pop-up.](/img/grandma3/2-4/popup_store-settings_v2-4-eb64dc.png)
...
In the title bar, there are three buttons. Tap Save Preferences to store the current settings as the new default user store preferences. Tap Load Preferences to load the stored user store preferences values into the store settings pop-up. The last button is used to [change the menu location](/grandma3/2-4/wvm_change_location/).
...
- **Output**:\
  Uses the status of the current output from the console as the source. The output can be affected by masters and DMX profiles. The [DMX sheet](/grandma3/2-4/patch_dmx_sheet/) shows what the output actually is. The **Use Selection** setting is limited to **All For Selected** or **All** (Default).
- **DMX**:\
...
They are described in the [Create Presets topic](/grandma3/2-4/presets_create/) or in the other [Preset topics](/grandma3/2-4/presets/).
...
Learn more about **Tracking** in the [What is Tracking](/grandma3/2-4/cue_tracking/) topic.

Learn more about **Cue Only** in the [Cue Only topic](/grandma3/2-4/cue_tracking_cue-only/).
...
The shield function can be used to protect tracked values in future cues. This setting is only relevant when the Cue mode is Tracking. The three options are explained in the [Tracking Shield topic](/grandma3/2-4/cue_tracking_shield/).

### Grid

There is one setting in this area. It is Merge Mode. This is relevant for all elements that store the fixture grid position - Learn more about the grid in the [Selection Grid topic](/grandma3/2-4/operate_selection/).
...
Here are the general cue timings and the preset timings. Read more about these different timings and what they do in the [Cue Timing topic](/grandma3/2-4/cue_timing/).
